...
def make_request(database):
    ...
    def calculate_cost(word_tuple, alerts=False):
        ...
        edge_count = list(db.execute("SELECT count FROM edges \
            WHERE word_u = ? AND word_v = ?", db_load.sort_words(word_tuple)))
        if not edge_count:
            if alerts:
    ...
    while True:
        ...
        request_stems = tuple([stemmer.stemWord(word).lower() for word in request \
            if word.lower() not in db_load.stop_words])
...
def main():
    ...
    if len(sys.argv) == 1:
        print("If no database exists, enter name for new database.")
        corpus_database = input("Database: ")
        print()
    else:
        corpus_database = sys.argv[1]

    corpus_documents = ['melville-moby_dick.txt','austen-emma.txt', 'bible-kjv.txt', \
        ...
        'chesterton-thursday.txt', 'edgeworth-parents.txt', 'reuters', 'brown']

    db_load.load_corpus(corpus_database, corpus_documents)
